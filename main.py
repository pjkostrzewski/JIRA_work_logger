...
if __name__ == '__main__':
    ...
    app = App(title="JIRA log work", 
              width=460, height=400, 
              bg="white", layout="grid")
    app.text_color = "black"
    input_login = TextBox(app, username, 
                          width=20, grid=[1, 1])
    input_password = tkinter.Entry(show="*", 
                                   width=20)
    password_widget = app.add_tk_widget(input_password, grid=[1, 2])
    calendar = tkcalendar.Calendar(background="#034DA4", 
                                   selectbackground="#034DA4")
    calendar_widget = app.add_tk_widget(calendar, 
                                        grid=[0, 1, 1, 8])
    issues = Combo(app, options=issues, grid=[1, 9])
    slider = Slider(app, start=1, end=8, 
                    grid=[1, 4])
    comment = TextBox(app, "<comment>", 
                      width=30, height=4, 
                      grid=[0, 10], 
                      multiline=True)
    comment.text_size = 8
